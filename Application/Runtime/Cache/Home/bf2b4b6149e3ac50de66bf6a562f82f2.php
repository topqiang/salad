...
<body>
	...
	<div class="con-wrap-bt">
		<div class="addr-outer">
			<ul>
				<li class="fl-clr">
					<input type="text" placeholder="请填写姓名" name="linkname" addid="<?php echo ($address["id"]); ?>" id="linkname" class="addr-inp name-inpt inpt" value="<?php echo ($address["name"]); ?>">
					<dl class="sex fl-clr">
						<dt><span class="sexval <?php if($address['sex'] == '男'): ?>on<?php endif; ?>"><font id="_man" style="">男</font></span></dt>
						<dd><span class="sexval <?php if($address['sex'] == '女'): ?>on<?php endif; ?>"><font id="_woman" style="">女</font></span></dd>
					</dl>
				</li>
				<li>
					<input type="text" placeholder="请填写手机号" name="mobile" value="<?php echo ($address["tel"]); ?>" id="mobile" class="addr-inp inpt" maxlength="11">
				</li>
				<li>
					<div class="addr-select" id="citySelect" onclick="$('#CitySelectLayer').show();" value="<?php echo ($address["provice"]); ?>">北京</div>
				</li>
				<li>
					<div class="addr-select" id="selectAddressLine" onclick="Jump();"><?php if(empty($line)): echo ($address["detailadd"]); else: echo ($line); endif; ?></div>
				</li>
				<li>
					<input type="text" placeholder="楼号、单元号和门牌号" id="doorcode" name="doorcode" class="addr-inp inpt" value="<?php echo ($address["numhouse"]); ?>">
				</li>
				<li>
					<div class="addr-select" id="tagSelect" onclick="$('#TagSelectLayer').show();"><?php echo ($address["label"]); ?></div>
				</li>
			</ul>
		</div>
	</div>
	<div class="bottom-btn">
		...
		<a href="<?php echo U('Address/del');?>/id/<?php echo ($address["id"]); ?>">
...
<script language="javascript">
	...
	function Jump(){
		...
		if (citySelect || citySelect != "") {
			...
			window.location.href = "<?php echo U('Address/inputarea');?>/city/"+citySelect+"/edi/edi/line/"+((line == "请选择详细地址") ? "" : line);
		}else{
			alert("请先选择城市");
		}
	}
