 ...
 ******************************************************
 *private function request()		取消POST数据转换成字符串形式,POST数据以数组方式直接提交
 *private function autoRedirect()	将表单元素隐藏域(input hidden)的value值改成单引号(')包裹
 *private function reactResponse()	原本有一个查询接口无值返回
 *
 */
class Chinapnr{
	 ...
	 * @param string $merId
	 */
	public function Chinapnr($params){
		...
		//$npcConfig = include('chinapnr.conf.php');
		$this->npcServerIp = $params['server'];
		$this->npcServerPort = $params['port'];
	...
	private function reactResponse($res= "", $signKeys=array()){
		$arrayFlag = json_decode($res,true);
		foreach($arrayFlag as $item) {
			if(is_array($item)) {
				$arrayFlag = true;
				break;
			}
		...
		if($arrayFlag !== true) {
			$ret = array_map('urldecode',json_decode($res,true));
		}else {
			$res = urldecode($res);
	 ...
	 * @return string
	 */
	public function sign($msgData){
		dump('传入到sign中的:'.$msgData);
		//return $this->scureTool->sign($signData);
		try {
			$fp = stream_socket_client($this->npcServerIp.":".$this->npcServerPort, $errno, $errstr, 5);
			if (!$fp) {
				return "$errstr ($errno)";
			}
			$type = 'S';
			$paramVal = $type.$this->merId.sprintf("%04d", strlen($msgData)).$msgData;
			$paramVal = sprintf("%04d", strlen($paramVal)).$paramVal;


			//$paramVal = $msgData;
			dump('sign准备加密:'.$paramVal);
			dump('sign应该加密:'.'0074S9900990059CashReconciliation00060000600002024272014081120140812110000');
			fputs($fp, $paramVal);
			$chkValue = '';
			while (!feof($fp)) {
				$chkValue .= fgets($fp, 128);
			}
			//dump('实际加密后获取的:'.$chkValue);
	 ...
	 * @return boolean
	 */
	private function verify($data,$chkValue){
		...
		try {
			...
			$type = 'V';
			$paramVal = $type.$this->merId.sprintf("%04d", strlen($data)).$data;
			$paramVal .= $chkValue;
			$paramVal = sprintf("%04d", strlen($paramVal)).$paramVal;
			fputs($fp, $paramVal);
			$response = '';
			...
			fclose($fp);
			return substr($response,0,4) === "0000"?true:false;
		} catch (Exception $e) {
			return $e->getMessage();
		}
	}
